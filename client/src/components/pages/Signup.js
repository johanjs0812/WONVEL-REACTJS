...
const Signup = () => {
    ...
    const { signup, signupError } = useApi();
    ...
    const validate = () => {
        ...
        if (!username) {
            ...
            newErrors.username = 'Tên người dùng phải có tối thiểu 6 ký tự';
        }

        if (!phone) {
            newErrors.phone = 'Không được để trống';
        } else if (phone.length < 10) {
            newErrors.phone = 'Số điện thoại tối thiểu phải có 10 số';
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await signup(username, phone, gmail, password);
